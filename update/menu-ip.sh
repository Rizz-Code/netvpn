...
adm=$(grep -E "^### " "/root/allow/ip" | cut -d ' ' -f 5 | sed -n "$nombor"p) #ip
sed -i "s/### $name1 $exp $ivps1 $adm//g" /root/allow/ip &> /dev/null
...
Isadmin=$(curl -sS https://accessvpn.onrender.com | grep $MYIP | awk '{print $5}')
if [ "$Isadmin" = "OFF" ]; then
clear
echo -e "$COLOR1┌─────────────────────────────────────────────────┐${NC}"
